...
class IfExecutor() : Executor<IfStatement>{
    ...
    override fun execute(ast: IfStatement, variables: MutableMap<String, Value>): String {
        when(ast.condition) {
    ...
    private fun evaluateVariable(variable : Token, variables : MutableMap<String , Value> ){
        if (!variables.containsKey(variable.getValue())) throw Exception("Variable ${variable.getValue()} not found at ${variable.getInitialPosition().first} : ${variable.getInitialPosition().second}")

        val value = variables[variable.getValue()]

        if (value != null) {
            if (value.getType() != VariableType.BOOLEAN) throw Exception("Variable ${variable.getValue()} is not a boolean at ${variable.getInitialPosition().first} : ${variable.getInitialPosition().second}")
        } else {
            throw Exception("Variable ${variable.getValue()} does not have a value, at ${variable.getInitialPosition().first} : ${variable.getInitialPosition().second}")
        }
    }
